...
+
Wenn eine Host-Port-Kennung nicht in der Liste angezeigt wird, bedeutet dies, dass sich der Host-Port nicht angemeldet hat. Mithilfe eines HBA-Dienstprogramms oder des iSCSI-Initiator-Dienstprogramms können die Host-Port-IDs gefunden und dem Host zugeordnet werden.
...
Der Standard-Alias ist <``Hostname_Port Number``>. Beispiel: Der Standard-Alias für den ersten Port, der für erstellt `host IPT is IPT_1` wurde.
