...
Server-URL
 a| 
Geben Sie die URL für den Zugriff auf den LDAP-Server in Form von `ldap[s]://*host*:*port*`ein.
...
Attribut Benutzername
 a| 
Geben Sie das Attribut ein, das zur Authentifizierung an die Benutzer-ID gebunden ist. Zum Beispiel: `sAMAccountName`.
...
Gruppenattribut\(s\)
 a| 
Geben Sie eine Liste der Gruppenattribute für den Benutzer ein, die für die Zuordnung von Gruppen zu Rollen verwendet werden. Zum Beispiel: `memberOf, managedObjects`.
...
Gruppen-DN
 a| 
Geben Sie den Group Distinguished Name (DN) für die zu zugeordnete LDAP-Benutzergruppe an. Reguläre Ausdrücke werden unterstützt. Diese Sonderzeichen mit regulären Ausdrücken müssen mit einem umgekehrten Schrägstrich versehen werden(`\`, wenn sie nicht Teil eines regulären Ausdrucksmusters sind: \.[]{}()<>*+-=!?^
