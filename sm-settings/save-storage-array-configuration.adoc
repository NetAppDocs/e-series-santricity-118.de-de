...
+
Die Datei wird im Ordner Downloads Ihres Browsers mit dem Namen gespeichert `storage-array-configuration.cfg`.



.Nachdem Sie fertig sind
Um die gespeicherte Speicher-Array-Konfiguration auf ein anderes Speicher-Array zu laden, verwenden Sie die SANtricity-Befehlszeilenschnittstelle (SMcli) mit der `-f` Option, die Datei anzuwenden `.cfg`.
