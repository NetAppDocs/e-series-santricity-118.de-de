...
** *Add* -- Verwenden Sie Add, um dem Host eine neue Host-Port-ID zuzuordnen. Die Länge des Namens der Host-Port-Kennung wird durch die Host-Schnittstellentechnologie bestimmt. Fibre-Channel- und Infiniband-Host-Port-Identifier-Namen müssen 16 Zeichen lang sein. ISCSI-Host-Port-Identifier-Namen haben maximal 223 Zeichen. Der Port muss eindeutig sein. Eine bereits konfigurierte Portnummer ist nicht zulässig.
