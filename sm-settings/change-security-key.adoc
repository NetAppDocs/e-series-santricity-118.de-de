...
+
** *Sicherheitsschlüssel-ID definieren* -- (nur für interne Sicherheitsschlüssel.) Übernehmen Sie den Standardwert (Name und Zeitstempel des Speicherarrays, der von der Controller-Firmware generiert wird), oder geben Sie Ihren eigenen Wert ein. Sie können bis zu 189 alphanumerische Zeichen ohne Leerzeichen, Satzzeichen oder Symbole eingeben.
+
[NOTE]
====
